...
import 'package:docx_creator/src/reader/models/docx_style.dart';
...
void main() {
  group('Rendering Fidelity', () {
    ...
    testWidgets('ParagraphBuilder renders centered floating image',
      ...
      final colFinder = find.byType(Column);

      // Verify we have a column that is centered
      final centeredCols = tester
          .widgetList<Column>(colFinder)
          .where((c) => c.crossAxisAlignment == CrossAxisAlignment.center);
      expect(centeredCols, isNotEmpty,
          reason: 'Should find a centered Column for floating image');
    ...
    testWidgets('ParagraphBuilder renders side floating images in Row',
      ...
      expect(rowWidget.children[0], isA<Image>()); // Image first
    });
  });
}
